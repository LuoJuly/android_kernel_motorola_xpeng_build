...
LOG=/data/local/tmp/wlan_crc_match_210.log
...
while [ "$i" -lt 20 ]; do
	if [ -f "$KO_VENDOR" ] || [ -f "$KO_MOD" ]; then
		# Prefer strings check for 5.4.210 marker once file exists
		if strings "$KO_VENDOR" 2>/dev/null | grep -q '5\.4\.210'; then
			break
		fi
		if strings "$KO_MOD" 2>/dev/null | grep -q '5\.4\.210'; then
			break
		fi
	fi
	sleep 1
...
pick_ko() {
	if [ -f "$KO_VENDOR" ] && strings "$KO_VENDOR" 2>/dev/null | grep -q '5\.4\.210'; then
		echo "$KO_VENDOR"
		return
	fi
	if [ -f "$KO_MOD" ]; then
